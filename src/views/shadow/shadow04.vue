...
const pointLight = new THREE.PointLight( 0xffffff, 0.6 ); // 点光源
...
// 设置阴影贴图的分辨率
pointLight.shadow.mapSize.set(9216, 9216)
pointLight.distance = 0
pointLight.decay  = 1 // 需要设置物理渲染器
pointLight.intensity = 2 // 亮度调节

const gui = new dat.GUI()
gui.add(lightBall.position, 'x')
    .min(-5)
    .max(5)
    .step(0.01)
...
const renderFn = () => {
  ...
  lightBall.position.y = 2 + Math.cos(time * 10)
  // 设置阻尼
  controls.update()
  render?.render(scene, camera)
